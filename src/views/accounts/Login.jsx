...
const Login = () => {
  ...
  return (
    <div className="container flex justify-center items-center gap-12">
      <img
        className="w-[35rem] hidden xl:block"
      ...
      <div className="w-[80%] md:w-[400px] mt-4 md:px-0 md:mt-4 flex flex-col">
        ...
        />
        <span className="text-pink-700 text-sm mt-1 italic">
          Email Tidak Valid
        </span>
        <div className="md:w-[400px] w-full h-24 relative">
          ...
          />
          <button
            onClick={() => setIsPasswordHidden((prev) => (prev = !prev))}
            className="absolute top-8 right-10 w-4"
          >
            {isPasswordHidden ? (
              <svg
                className="w-7"
          ...
          <span className="absolute text-sm top-20 italic text-pink-700">
            Password Salah
          </span>
        </div>
        <button
          onClick={submit}
          className="mt-6 w-32 mx-auto text-white font-semibold bg-blue-500 py-3 rounded-full hover:bg-blue-600 delay-75 transition"
        >
          Login
        </button>
        <span className="text-center text-gray-500 mt-3">
          Belum punya akun?{" "}
          <a href="/accounts/signin" className="font-semibold text-blue-500">
            Daftar
          </a>
        </span>
      </div>
